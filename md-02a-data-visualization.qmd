...
*continent -\> non-numerical categorical variables*

*Life expectancy -\> numerical, discrete variables*

*colour -\> either numerical (defined by a number: RGB,HEX,... ) or non-numerical categorical variables( defined by Name: green, red, ...)*
...
ggplot(data = gapminder_2007,
       mapping = aes(x = continent,
                     y = lifeExp,
                     fill = continent)) +
  geom_boxplot(outlier.shape = NA) # source : https://www.perplexity.ai/search/how-do-i-mV_9NDKRQy6p7dxhtK05NA#0
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    *The geom_bar() function counts the number of people of each categorical value, in this case continents. with the geom_col() function the data was already counted in "gapminder_summary_2007".*
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    *Both functions are counting the numbers of people in one category and plot them.*
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    *They position the plot on the x-Axis.*

    *The first value (40) marks the starting point of the x-Axis and the second value (85) the end point of the x-Axis and the third value (2.5) defines the delta_x value of the displayed colum width.*

```{r}
ggplot( data = gapminder_2007 , aes(x = lifeExp,
                                    fill = continent)) +
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    *The county_colors object is a color scheme which stores for every country a color.*
